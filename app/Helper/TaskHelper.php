...
class TaskHelper extends Base
{
    ...
    public function renderReferenceField(array $values, array $errors = array(), array $attributes = array())
    {
       ...
       public function renderAddressField(array $values, array $errors = array(), array $attributes = array())
    {
        $attributes = array_merge(array('tabindex="15"'), $attributes);

        $html = $this->helper->form->label(t('Address'), 'address');
        $html .= $this->helper->form->text('address', $values, $errors, $attributes, 'form-input-small');

        return $html;
    }

    public function renderCityField(array $values, array $errors = array(), array $attributes = array())
    {
        $attributes = array_merge(array('tabindex="15"'), $attributes);

        $html = $this->helper->form->label(t('City'), 'city');
        $html .= $this->helper->form->text('city', $values, $errors, $attributes, 'form-input-small');
 ...
 public function renderAddress(array $task)
    {
        if (! empty($task['address'])) {
            $address = $this->helper->text->e($task['address']);

            if (filter_var($task['address'], FILTER_VALIDATE_URL) !== false) {
                return sprintf('<a href="%s" target=_blank">%s</a>', $address, $address);
            }
    ...
    public function renderCity(array $task)
    {
        if (! empty($task['city'])) {
            $city = $this->helper->text->e($task['city']);

            if (filter_var($task['city'], FILTER_VALIDATE_URL) !== false) {
                return sprintf('<a href="%s" target=_blank">%s</a>', $city, $city);
            }
